...
app.put("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const foundJokeIdx = jokes.findIndex((joke) => joke.id === id);
  const newJoke = {
    id: id,
    jokeText: req.body.text,
    jokeType: req.body.type,
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) {
    ...
    res
      .status(404)
      .json({ error: `joke with id ${id} not found, Joke not deleted.` });
  }
});
//8. DELETE All jokes
app.delete("/all", (req, res) => {
  ...
  if (userKey === masterKey) {
    ...
    res.status(404).json({ error: `unauthorised to perform this action` });
  }
});
